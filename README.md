...
- **當週概覽**：固定顯示當週（一～日）七格，每天顯示請假人數，哪天有部門將達 1/3 上限直接標紅點 —— 事前預警，不是事後才知道。點任一天即切換檢視，週列不跟著捲。
- **部門人力 × 休假名單整併**：依部門分組時，每個部門標題列直接帶請假占比條（達 1/3 標紅、只差一人標黃），底下接該部門休假名單，缺人的部門一眼跳出來。
- 可按**部門篩選**，休假人員可依**部門或假別分組**顯示，1/3 人力與職代警示（含**職代出差**）直接標在畫面上。
